...
<table class="table table-hover">
	<tr>
		...
		<th class="col-md-2">Column Index</th>
		<th class="col-md-2">Cell Value</th>
	...
	if(isset($_GET['cari'])){
		...
		$brg=mysql_query("SELECT Dataset.RowIndex, parameters.ColumnIndex, SUM(Weight * CellValue) as MatrixProductValue FROM Dataset CROSS JOIN parameters ON Dataset.ColumnIndex = Parameters.RowIndex GROUP BY Dataset.RowIndex, Parameters.ColumnIndex limit $start, $per_hal");
	}
	$no=1;
	while($b=mysql_fetch_array($brg)){
		...
		<tr>
			...
			<td><?php echo $b['ColumnIndex'] ?></td>
			<td><?php echo $b['MatrixProductValue'] ?></td>
			<td>
